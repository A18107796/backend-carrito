...
public interface DetalleComprasDAO extends JpaRepository<Detalle_compras, Integer> {
	...
	@Query("select sum(c.cantidad * c.subtotal) from Detalle_compras c")
	Double getGananciasTotales();

	@Query(nativeQuery = true, value = "select sum(a.subtotal * a.cantidad) from compras c join detalle_compras a on a.id_compras = c.id where c.fecha_compras BETWEEN ?1 and ?2")
